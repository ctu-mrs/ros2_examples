...
ServiceClientExample::ServiceClientExample(const rclcpp::NodeOptions& options) : Node("service_client_example", options) {
  ...
  timer_async_ =
      create_wall_timer(std::chrono::duration<double>(1.0 / 200.0), std::bind(&ServiceClientExample::callback_timer_async, this), timer_callback_group_);

  timer_sync_ =
      create_wall_timer(std::chrono::duration<double>(1.0 / 200.0), std::bind(&ServiceClientExample::callback_timer_sync, this), timer_callback_group_);
                                        ...
                                        /* this function will run in a separate thread under the default callback group of the node */
                                        {
                                          ...
                                          if (result->success) {
                                            RCLCPP_INFO_STREAM(get_logger(),
                                                               "Async callback called, service call successful with message: " << result->message);
                                          } else {
