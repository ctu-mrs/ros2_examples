...
SubscriberExample::SubscriberExample(rclcpp::NodeOptions options) : Node("subscriber_example", options) {
  ...
  const std::function<void(const std_msgs::msg::String::SharedPtr)> fast_cbk =
      std::bind(&SubscriberExample::callback_fast_subscriber, this, std::placeholders::_1);

  subscriber_fast_ = create_subscription<std_msgs::msg::String>("~/topic_fast_in", 10, fast_cbk, sub_opt);

  const std::function<void(const std_msgs::msg::String::SharedPtr)> slow_cbk =
      std::bind(&SubscriberExample::callback_slow_subscriber, this, std::placeholders::_1);

  subscriber_slow_ = create_subscription<std_msgs::msg::String>("~/topic_slow_in", 10, slow_cbk, sub_opt);

  const std::function<void(const std_msgs::msg::String::SharedPtr)> irregular_cbk =
      std::bind(&SubscriberExample::callback_irregular_subscriber, this, std::placeholders::_1);
...
void SubscriberExample::callback_fast_subscriber(const std_msgs::msg::String::SharedPtr msg) {
  ...
  RCLCPP_INFO_STREAM_THROTTLE(get_logger(), *get_clock(), 1000, "receiving string message '" << msg->data << "' at rate " << rate << "Hz.");
}
